...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-full h-screen bg-[#05060f] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden text-slate-100" id="login-root-view">
      
      {/* Dynamic drifting background lighting for 3D effect */}
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div className="flex justify-center">
          <motion.div 
            ...
            className="w-12 h-12 rounded-xl bg-gradient-to-tr from-amber-400 via-rose-450 via-rose-400 to-violet-500 flex items-center justify-center text-slate-950 font-black text-xl tracking-tighter"
          >
            vC
          </motion.div>
        </div>
        <h2 className="mt-5 text-center text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-white to-violet-350 bg-gradient-to-r from-amber-300 via-white to-violet-400 font-sans">
          VVCV Creative
        </h2>
        <p className="mt-2 text-center text-xs text-slate-400">
          Sign in to access your beautiful design canvas environment
        </p>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 px-4">
        {/* Sleek card featuring a rich glassmorphism border and 3D shadow depth */}
        <div className="bg-[#0a0d1b]/85 border border-white/[0.05] shadow-2xl shadow-indigo-950/20 rounded-3xl p-6 sm:p-8 space-y-6">
          ...
          {/* Social Sign In Buttons with brand alignment */}
          ...
          <div className="relative flex py-2 items-center">
            ...
            <span className="flex-shrink mx-3 text-[10px] font-mono text-slate-600 uppercase tracking-wider">or sign in securely</span>
            <div className="flex-grow border-t border-slate-900"></div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-[#04060d] border border-slate-900 hover:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-violet-400/40 focus:ring-1 focus:ring-violet-400/20"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-amber-400 via-rose-400 to-violet-500 text-slate-950 font-extrabold py-2.5 rounded-xl text-xs flex items-center justify-center gap-2 hover:opacity-95 active:scale-[0.99] transition-transform cursor-pointer mt-2 shadow-lg shadow-amber-500/5"
            >
              {loading ? (
